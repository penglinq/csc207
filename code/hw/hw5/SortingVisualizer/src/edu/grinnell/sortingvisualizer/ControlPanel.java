    ...
    public ControlPanel(NoteIndices notes, ArrayPanel panel) {
        ...
        playButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                Integer[] arrcp = Arrays.copyOf(notes.getNotes(), notes.getNotes().length); 
                final List<SortEvent<Integer>> events = generateEvents((String) sorts.getSelectedItem(), arrcp);
                /*int index = events.size() - 1;
                while (index >= 0) {
                	if (!events.get(index).isEmphasized()) {
                		events.add(events.get(index));
                		events.remove(index);
                	}
                	index--;
                }*/
